...
**Subplans recurse.** `@subplans` is a list of `MediaPlan`s, so a subplan is the
same object serialized the same way, one level down; the writer and reader
recurse and need nothing new per level.
