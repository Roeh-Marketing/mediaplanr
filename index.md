...
A channel team’s detailed plan — at its own grain, on its own calendar —
can be attached beneath one cell of the topline with
[`attach_subplan()`](https://roeh-marketing.github.io/mediaplanr/reference/attach_subplan.md).
The cell’s rows become the subplan’s rollup and are read-only until
detached, so there is one number with one owner and re-attaching is the
reconcile. See
...
for what is next and for what is deliberately staying out — a
