...
A flight has a **shape** as well as a size. By default its spend is
spread evenly across its days, so its weekly figures are whatever that
gives — which for a buy with ragged ends is not four equal numbers:
...
#> [1] 19230.77 26923.08 26923.08 26923.07
flights(ragged)$pacing
#> [1] "even"
```

`pacing` is **re-derived after every edit**, not remembered. The
question it answers is “do these rows still match an even spread?”,
never “did somebody touch this?” — and that distinction is what makes it
behave sensibly:
...
Because it is derived rather than remembered, shaping a flight by hand
and then shaping it back reports `"even"` again. There is no permanent
mark.

A hand-shaped flight is never **broken** by the edit: it keeps its
identity, so
[`flights()`](https://roeh-marketing.github.io/mediaplanr/reference/flights.md)
still reports it whole and it can still be compared, moved and restaged
as one buy. `"custom"` says the shape is no longer a rule, not that the
buy has stopped being a buy.

### The one place the package rewrites a number

Worth knowing because it is the exception to everything else here. When
a flight is still evenly paced, its rows are **snapped** to the
canonical allocation — the same figures a fresh import would produce.

The reason is drift. Scaling values already rounded to the cent does not
land on the cent-exact allocation of the scaled total, so without
snapping a long chain of scenarios would accumulate fractions of a cent
and slowly stop being even at all. With it, the chain is stable:
...
The snap only touches flights already even to within a rounding
tolerance, moves at most a cent between weeks, and **never changes a
flight’s total**. A hand-shaped flight is left exactly as the planner
set it.
